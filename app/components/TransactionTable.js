...
export default function TransactionTable({ transactions, expenseCats = [], capitalCats = [] }) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setDeletingId(null);
      setIdToDelete(null);
      setSwipedId(null); // Reset swipe state
    }
  }
  ...
  return (
    <section className="flex flex-col gap-6 relative w-full">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="w-full max-w-sm rounded-[32px] border border-white/10 bg-[#161b27]/95 p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)] backdrop-blur-2xl ring-1 ring-white/20">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmDelete} className="click-pop flex-1 rounded-2xl bg-rose-600/20 border border-rose-500/50 py-3.5 text-[10px] font-black italic tracking-widest text-rose-400 uppercase transition-all shadow-[0_0_20px_rgba(244,63,94,0.2)]">Delete</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="w-full max-w-md rounded-[32px] border border-white/10 bg-[#161b27]/95 p-6 shadow-[0_0_50px_rgba(0,0,0,0.8)] backdrop-blur-2xl ring-1 ring-white/20">
            <div className="mb-6 flex items-center gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-500/10 text-sky-400 border border-sky-500/20 shadow-[0_0_15px_rgba(56,189,248,0.3)]">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                </svg>
              </div>
              <div>
            ...
            <form onSubmit={handleUpdate} className="flex flex-col gap-4">
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button type="submit" disabled={isUpdating} className="click-pop flex-1 rounded-2xl bg-sky-600/20 border border-sky-500/50 py-3.5 text-[10px] font-black italic tracking-widest text-sky-400 uppercase transition-all shadow-[0_0_20px_rgba(56,189,248,0.2)] hover:bg-sky-500/30">
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-3 w-full sm:w-auto">
          ...
          <div className="flex gap-1.5 sm:gap-2 bg-[#161b27]/30 backdrop-blur-md p-1 rounded-2xl border border-white/5 w-full sm:w-auto">
            {["all", "income", "expense"].map((f) => (
              <button key={f} onClick={() => { haptic.light(); setFilter(f); }} className={`flex-1 sm:flex-none rounded-xl px-2 sm:px-4 py-1.5 sm:py-2 text-[8px] sm:text-[10px] font-black italic tracking-widest uppercase transition-all ${filter === f ? 'bg-sky-500/20 text-sky-400' : 'text-slate-500'}`}>
                {f}
              </button>
            ))}
          </div>
      ...
      {/* 🚀 Data List with Swipe-to-Delete */}
      <div className="rounded-[24px] sm:rounded-[40px] border border-white/5 scroll-glass p-3 sm:p-5 flex flex-col gap-2 w-full">
        {filtered.map((txn, idx) => (
          <div
            ...
            // Swipe Events
            onPointerDown={(e) => { rowStartX.current = e.clientX; }}
            onPointerMove={(e) => {
              if (rowStartX.current === null) return;
              const dx = e.clientX - rowStartX.current;
              // Swipe Left පමණක් ගණන් ගනී
              if (dx < 0 && swipedId !== txn.id) {
                setDeleteX(Math.max(dx, -90)); // උපරිම 90px ඇදෙන්න දෙනවා
              }
            }}
            onPointerUp={() => {
              if (deleteX < -60) {
                haptic.medium(); // Vibrate when swiped
                setSwipedId(txn.id);
              } else {
                setDeleteX(0); // ආයෙත් පරණ තැනටම යනවා
                setSwipedId(null);
              }
              rowStartX.current = null;
            }}
            onPointerLeave={() => {
              if (swipedId !== txn.id) setDeleteX(0);
              rowStartX.current = null;
            }}
          >
            {/* 🔴 Delete Zone (පිටිපස්සේ හැංගිලා තියෙන එක) */}
            <div
              ...
              style={{ opacity: swipedId === txn.id ? 1 : Math.abs(deleteX) / 100 }}
            >
              <button onClick={() => askDelete(txn.id)} className="click-pop w-full h-full flex flex-col items-center justify-center text-rose-400 gap-1">
            ...
            {/* 🟢 Content (ඇදෙන කෑල්ල) */}
            <div
              className="flex items-center justify-between p-3 sm:p-4 transition-transform bg-[#161b27]/40 cursor-grab active:cursor-grabbing"
              style={{ transform: `translateX(${swipedId === txn.id ? -90 : deleteX}px)`, transition: rowStartX.current ? 'none' : 'transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)' }}
            >
              <div className="flex items-center gap-3 sm:gap-4 overflow-hidden" onClick={() => { if (swipedId !== txn.id) openEdit(txn); }}>
