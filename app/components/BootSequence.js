...
export default function BootSequence({ onComplete }) {
    const [stage, setStage] = useState(0); // 0=black, 1=logo, 2=reveal

    useEffect(() => {
        const t1 = setTimeout(() => setStage(1), 100);
        const t2 = setTimeout(() => setStage(2), 1200); // 🚀 ටිකක් ඉක්මන් කළා පෝන් එකට
    ...
    return (
      ...
      ${stage >= 2 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            ...
            {/* Logo Mark */}
            <div className={`relative transition-all duration-[800ms] cubic-bezier(0.2, 0.8, 0.2, 1) ${stage >= 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
